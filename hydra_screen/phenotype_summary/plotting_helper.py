...
def make_clustermaps(featZ, meta, featsets, strain_lut, feat_lut, group_vars=['worm_gene','imaging_date_yyyymmdd'], saveto=None):
    """
    Parameters
    ----------
    featZ : TYPE
    ...
    if add_stats:
        if bhP_values_df is not None:
            add_stat_annotation(ax,
                                ...
                                loc='outside',
                                verbose=2,
                                text_annot_custom=['p={}'.format(round(bhP_values_df[feature].values[0],10))], #:.4f
                                fontsize=20
                                )
        else:
            import itertools
