...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 28 09:04:07 2020

@author: ibarlow

Analysis of the myosin mutants
...
if __name__ == '__main__':
    ...
    set(meta.imgstore_name.unique()) - set(fname.filename.unique())
    ...
    #%% wells to check
    nan_worms = meta[meta.worm_gene.isna()][['featuresN_filename', 'well_name']]
    ...
    myo_locs = list(meta.query('@myosin_genes in worm_gene').index)
    myo_dates = meta.loc[myo_locs, 'imaging_date_yyyymmdd'].unique()
    
    N2_locs = meta.query('@myo_dates in imaging_date_yyyymmdd and @CONTROL_STRAIN in worm_gene').sample(300).index

    myosin_genes.append(CONTROL_STRAIN)
    myo_locs.extend(N2_locs)    

    cmap = list(np.flip(sns.color_palette('cubehelix', len(myosin_genes)-1)))
    cmap.append([0.6, 0.6, 0.6])
    
    lut = dict(zip(myosin_genes, cmap))

     #%% Only do analysis on the disease strains    
    meta = meta.loc[myo_locs,:]
    feat = feat.loc[myo_locs,:]
    ...
    for fset in featsets.keys():
        myo_feats[fset], scores, support = k_significant_feat(
            feat_nonan.loc[:,featsets[fset]],
            meta.worm_gene,
            k=20,
            plot=False)
    ...
    for f in myo_feats:
        ...
        for s in myo_feats[f]:
            plt.figure()
            sns.boxplot(x=feat.loc[:, s],
                        y=meta.loc[:, 'worm_gene'],
                        order=myosin_genes,
                        orient='h',
                        showfliers=False,
                        palette = cmap)
...
#%% now paired stats tests
    ...
    for c,g in enumerate(myosin_genes):
        if g == 'N2':
            continue
        else:
            ...
            (SAVETO / 'pairwise_stats_test' / g / 'top10').mkdir(exist_ok=True)
            for f in _top[:10]:
                ...
                plt.savefig(SAVETO / 'pairwise_stats_test' / g / 'top10' / \
                            '{}.png'.format(f))
                plt.close('all')
    
            #plot bluelight comparisons for top 40 different features
            ...
            for f in _bluelight:
                ...
                plot_df = pd.concat([feat.loc[:, flist],
                                    meta], axis=1)     
    ...
    for fset in featset256:
        ...
        plt.title(fset)
        # plt.tight_layout()
        plt.savefig(SAVETO / '{}_256feats_{}_clustermap.png'.format(fset, subgroup),
                    dpi=400)
    ...
    g = sns.clustermap(feat_nonan[feat256_allconditions],
                        ...
                        xticklabels=False,
                        yticklabels=False)
    plt.savefig(SAVETO / 'allconditions_256feats_{}_clustermap.png'.format(subgroup),
                dpi=400)
...
# %% hierachical clustering by avearging the strains for each plate
    ...
    for fset in featset256:
        ...
        plt.title(fset)
        # plt.tight_layout()
        plt.savefig(SAVETO / '{}_averaged_by_plat_256feats_{}_clustermap.png'.format(fset, subgroup),
                    dpi=400)
    ...
    g = sns.clustermap(feat_av_grouped[feat256_allconditions],
                        ...
                        xticklabels=False,
                        yticklabels=False)
    plt.savefig(SAVETO / 'allconditions_averaged_by_plate_256feats_{}_clustermap.png'.format(subgroup),
                dpi=400)
